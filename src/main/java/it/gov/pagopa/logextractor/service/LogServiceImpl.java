...
public class LogServiceImpl implements LogService {
  ...
  public BaseResponseDto getAnonymizedPersonLogs(PersonLogsRequestDto requestData,
    ...
    if (requestData.getDateFrom() != null && requestData.getDateTo() != null
      ...
      if (requestData.getIun() != null) {
        ...
        NotificationDetailsResponseDto notificationDetails = notificationApiHandler.getNotificationDetails(
            requestData.getIun());
        ...
        OffsetDateTime notificationStartDate = OffsetDateTime.parse(
            notificationDetails.getSentAt());
        String notificationEndDate = notificationStartDate.plusMonths(3).toString();
        performanceMillis = System.currentTimeMillis();
        openSearchResponse = openSearchApiHandler.getAnonymizedLogsByIun(requestData.getIun(),
            notificationStartDate.toString(), notificationEndDate);
      }
    }
    log.info(LoggingConstants.QUERY_EXECUTION_COMPLETED_TIME,
  ...
  public BaseResponseDto getMonthlyNotifications(MonthlyNotificationsRequestDto requestData,
    ...
    String encodedPublicAuthorityName = deanonimizationApiHandler.getPublicAuthorityId(
        requestData.getPublicAuthorityName());
    log.info(
        "Public authority id retrieved in {} ms, getting notifications, publicAuthority={}, startDate={}, "
            + "endDate={}", System.currentTimeMillis() - performanceMillis,
        encodedPublicAuthorityName, requestData.getReferenceMonth(), requestData.getEndMonth());
    performanceMillis = System.currentTimeMillis();
    ...
    log.info("{} notifications retrieved in {} ms, constructing service response...",
        notifications.size(), System.currentTimeMillis() - performanceMillis);
    ...
    performanceMillis = System.currentTimeMillis();
    int numberOfFiles = (int) Math.ceil(
        ((double) notifications.size()) / GenericConstants.CSV_FILE_MAX_ROWS);
    ...
    while (numberOfFiles > 0) {
      ...
      File file = fileUtils.getFileWithRandomName(GenericConstants.NOTIFICATION_CSV_FILE_NAME,
          GenericConstants.CSV_EXTENSION);
      fileUtils.writeCsv(file, fileUtils.toCsv(notificationsPartition));
      csvFiles.add(file);
      numberOfFiles--;
    }
    DownloadArchiveResponseDto response = constructor.createCsvFileResponse(csvFiles,
        GenericConstants.ZIP_ARCHIVE_NAME);
    log.info(LoggingConstants.SERVICE_RESPONSE_CONSTRUCTION_TIME,
        System.currentTimeMillis() - performanceMillis);
    log.info("Monthly notifications retrieve process - END in {} ms",
        (System.currentTimeMillis() - serviceStartTime));
  ...
  @Override
  public BaseResponseDto getTraceIdLogs(TraceIdLogsRequestDto requestData, String xPagopaHelpdUid,
    ...
    List<String> openSearchResponse = openSearchApiHandler.getAnonymizedLogsByTraceId(
        requestData.getTraceId(), requestData.getDateFrom(), requestData.getDateTo());
    long performanceMillis = System.currentTimeMillis() - serviceStartTime;
    log.info(LoggingConstants.QUERY_EXECUTION_COMPLETED_TIME, performanceMillis,
        openSearchResponse.size());
    if (openSearchResponse.isEmpty()) {
      performanceMillis = System.currentTimeMillis();
      BaseResponseDto response = new BaseResponseDto();
      response.setMessage(ResponseConstants.NO_DOCUMENT_FOUND_MESSAGE);
  ...
  public BaseResponseDto getNotificationInfoLogs(NotificationInfoRequestDto requestData,
      String xPagopaHelpdUid, String xPagopaCxType) throws IOException {
    log.info(
        "Notification data retrieve process - START - user={}, userType={}, ticketNumber={}, iun={}",
    ...
    log.info("Service response: notificationDetails={} retrieved in {} ms, getting history data...",
        mapper.writer().writeValueAsString(notificationDetails),
        System.currentTimeMillis() - serviceStartTime);
    NotificationHistoryResponseDto notificationHistory = notificationApiHandler.getNotificationHistory(
        requestData.getIun(), notificationDetails.getRecipients().size(),
        notificationStartDate.toString());
    log.info(
        "Service response: notificationHistory={} retrieved in {} ms, getting legal facts' keys...",
        mapper.writer().writeValueAsString(notificationHistory),
        System.currentTimeMillis() - serviceStartTime);
    long performanceMillis = System.currentTimeMillis();
    ...
    log.info("Legal facts' keys retrieved in {} ms, getting notification documents' keys...",
        System.currentTimeMillis() - performanceMillis);
    performanceMillis = System.currentTimeMillis();
    downloadFileData.addAll(
        notificationApiHandler.getNotificationDocumentFileDownloadData(notificationDetails));
    log.info("Notification documents' keys retrieved in {} ms, getting payment documents' keys...",
        System.currentTimeMillis() - performanceMillis);
    performanceMillis = System.currentTimeMillis();
    downloadFileData.addAll(
        notificationApiHandler.getPaymentFilesDownloadData(notificationDetails));
    ...
    for (NotificationDownloadFileData currentDownloadData : downloadFileData) {
      try {
        FileDownloadMetadataResponseDto downloadMetaData = notificationApiHandler.getDownloadMetadata(
            currentDownloadData.getKey());
        currentDownloadData.setDownloadUrl(downloadMetaData.getDownload().getUrl());
        downloadableFiles.add(currentDownloadData);
        if (null != downloadMetaData.getDownload() && null == downloadMetaData.getDownload()
            .getUrl() && null != downloadMetaData.getDownload().getRetryAfter()
    ...
    if (secondsToWait > 0) {
      ...
      BaseResponseDto response = new BaseResponseDto();
      response.setMessage(
          ResponseConstants.OPERATION_CANNOT_BE_COMPLETED_MESSAGE + timeToWaitInMinutes + (
              timeToWaitInMinutes > 1 ? GenericConstants.MINUTES_LABEL
                  : GenericConstants.MINUTE_LABEL));
      log.info(LoggingConstants.SERVICE_RESPONSE_CONSTRUCTION_TIME,
          System.currentTimeMillis() - performanceMillis);
      ...
      for (NotificationDownloadFileData currentDownloadableFile : downloadableFiles) {
        filesToAdd.add(fileUtils.getFile(
            currentDownloadableFile.getFileCategory() + "-" + currentDownloadableFile.getKey(),
            GenericConstants.PDF_EXTENSION, currentDownloadableFile.getDownloadUrl()));
      }
      log.info("Physical files retrieved in {} ms", System.currentTimeMillis() - performanceMillis);
      List<String> openSearchResponse = openSearchApiHandler.getAnonymizedLogsByIun(
          requestData.getIun(), notificationStartDate.toString(), notificationEndDate);
      log.info(LoggingConstants.QUERY_EXECUTION_COMPLETED_TIME,
          System.currentTimeMillis() - performanceMillis, openSearchResponse.size());
      performanceMillis = System.currentTimeMillis();
      if (!openSearchResponse.isEmpty()) {
      ...
      DownloadArchiveResponseDto response = constructor.createNotificationLogResponse(filesToAdd,
          filesNotDownloadable, GenericConstants.ZIP_ARCHIVE_NAME);
      log.info(LoggingConstants.SERVICE_RESPONSE_CONSTRUCTION_TIME,
          System.currentTimeMillis() - performanceMillis);
      log.info("Notification data retrieve process - END in {} ms",
          (System.currentTimeMillis() - serviceStartTime));
  ...
  public BaseResponseDto getDeanonimizedPersonLogs(PersonLogsRequestDto requestData,
      String xPagopaHelpdUid, String xPagopaCxType) throws IOException, LogExtractorException {
    log.info(
    ...
    //use case 3
    if (requestData.getDateFrom() != null && requestData.getDateTo() != null
        && requestData.getTaxId() != null && requestData.getRecipientType() != null
        && requestData.getIun() == null) {
      log.info("Getting internal id...");
      String internalId = deanonimizationApiHandler.getUniqueIdentifierForPerson(
          requestData.getRecipientType(), requestData.getTaxId());
      log.info("Service response: internalId={} retrieved in {} ms", internalId,
          System.currentTimeMillis() - serviceStartTime);
      performanceMillis = System.currentTimeMillis();
      openSearchResponse = openSearchApiHandler.getAnonymizedLogsByUid(internalId,
          requestData.getDateFrom(), requestData.getDateTo());
      log.info(LoggingConstants.QEURY_EXECUTION_COMPLETED_TIME_DEANONIMIZE_DOCS,
          System.currentTimeMillis() - performanceMillis, openSearchResponse.size());
      performanceMillis = System.currentTimeMillis();
      deanonimizedOpenSearchResponse = deanonimizationApiHandler.deanonimizeDocuments(
          openSearchResponse, requestData.getRecipientType());
      if (!deanonimizedOpenSearchResponse.isEmpty()) {
        ...
        if (StringUtils.isNotBlank(date)) {
          String name = String.format("%s-%s", jsonUtils.getValue(openSearchResponse.get(0), "jti"),
              LocalDateTime.parse(date, DateTimeFormatter.ISO_OFFSET_DATE_TIME).toLocalDate()
                  .toString());
          log.info("Retrieving SAML assertion from s3 bucket... ");
          performanceMillis = System.currentTimeMillis();
          File assertion = s3ApiHandler.getFile(name, GenericConstants.JSON_EXTENSION);
          if (assertion != null) {
            filesToAdd.add(assertion);
            log.info(
                "SAML assertion from s3 bucket retrieved in {} ms, constructing service response...",
                System.currentTimeMillis() - performanceMillis);
          }
        }
        File logFile = fileUtils.writeTxt(deanonimizedOpenSearchResponse,
            GenericConstants.LOG_FILE_NAME);
        filesToAdd.add(logFile);
        return constructor.createNotificationLogResponse(filesToAdd, new ArrayList<>(),
            GenericConstants.ZIP_ARCHIVE_NAME);
      }
      ...
      if (requestData.getIun() != null) {
        ...
        performanceMillis = System.currentTimeMillis();
        deanonimizedOpenSearchResponse = deanonimizationApiHandler.deanonimizeDocuments(
            openSearchResponse, RecipientTypes.PF);
      }
    }
    ...
    performanceMillis = System.currentTimeMillis();
    DownloadArchiveResponseDto response = constructor.createSimpleLogResponse(
        deanonimizedOpenSearchResponse, GenericConstants.LOG_FILE_NAME,
        GenericConstants.ZIP_ARCHIVE_NAME);
    log.info(LoggingConstants.SERVICE_RESPONSE_CONSTRUCTION_TIME,
        System.currentTimeMillis() - performanceMillis);
    log.info("deanonimized logs retrieve process - END in {} ms",
  ...
  public BaseResponseDto getAnonymizedSessionLogs(SessionLogsRequestDto requestData,
    ...
    long performanceMillis = System.currentTimeMillis();
    openSearchResponse = openSearchApiHandler.getAnonymizedSessionLogsByJti(requestData.getJti(),
        requestData.getDateFrom(), requestData.getDateTo());
  ...
  public BaseResponseDto getDeanonimizedSessionLogs(SessionLogsRequestDto requestData,
      String xPagopaHelpdUid, String xPagopaCxType) throws IOException, LogExtractorException {
    log.info(
        "Deanonimized session logs retrieve process - START - user={}, userType={}, ticketNumber={}, "
            + "jti={}, startDate={}, endDate={}", xPagopaHelpdUid, xPagopaCxType,
        requestData.getTicketNumber(), requestData.getJti(), requestData.getDateFrom(),
        requestData.getDateTo());
    long serviceStartTime = System.currentTimeMillis();
    List<String> openSearchResponse;
    ...
    if (StringUtils.isNotBlank(date)) {
      ...
      log.info("Retrieving SAML assertion from s3 bucket... ");
      performanceMillis = System.currentTimeMillis();
      filesToAdd.add(s3ApiHandler.getFile(name, GenericConstants.JSON_EXTENSION));
    ...
    filesToAdd.add(logFile);
    DownloadArchiveResponseDto response = constructor.createNotificationLogResponse(filesToAdd,
        new ArrayList<>(), GenericConstants.ZIP_ARCHIVE_NAME);
    log.info(LoggingConstants.SERVICE_RESPONSE_CONSTRUCTION_TIME,
        System.currentTimeMillis() - performanceMillis);
    log.info(LoggingConstants.DEANONIMIZED_RETRIEVE_PROCESS_END,
        (System.currentTimeMillis() - serviceStartTime));
